...
async function main() {
    ...
    const files = await Promise.all(filesToGenerate.map(async (file) => {
        ...
        const spec = await code.toStringWithImports({ ...(0, options_1.getTsPoetOpts)(options), path });
        return { name: path, content: (0, utils_1.prefixDisableLinter)(spec) };
    }));
    if (options.outputTypeRegistry) {
        ...
        const spec = await code.toStringWithImports({ ...(0, options_1.getTsPoetOpts)(options), path });
        files.push({ name: path, content: (0, utils_1.prefixDisableLinter)(spec) });
    }
